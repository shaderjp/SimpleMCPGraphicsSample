...
Dear ImGui の操作パネルと stdio / Streamable HTTP MCP サーバーを組み込んだ実装です。
...
読みやすい出発点を目的としています。HTTP版はlocalhost専用で、認証、TLS、SSE、
Prompts、Tasks、Sampling などは意図的に含めていません。
...
| `SimpleMCPGraphicsSampleHttpD3D12` | Direct3D 12 + ImGui + Streamable HTTP MCP |
| `SimpleMCPGraphicsSampleHttpVulkan` | Vulkan + ImGui + Streamable HTTP MCP |
| `SimpleMCPGraphicsSampleTests` | 状態管理、JSON-RPC、stdio framing、HTTP/sessionの単体テスト |
...
| 版 | 実行ファイル |
| --- | --- |
| stdio D3D12 | `Bin\SimpleMCPGraphicsSampleD3D12\x64\Debug\SimpleMCPGraphicsSampleD3D12.exe` |
| stdio Vulkan | `Bin\SimpleMCPGraphicsSampleVulkan\x64\Debug\SimpleMCPGraphicsSampleVulkan.exe` |
| HTTP D3D12 | `Bin\SimpleMCPGraphicsSampleHttpD3D12\x64\Debug\SimpleMCPGraphicsSampleHttpD3D12.exe` |
| HTTP Vulkan | `Bin\SimpleMCPGraphicsSampleHttpVulkan\x64\Debug\SimpleMCPGraphicsSampleHttpVulkan.exe` |
...
### 4. Streamable HTTP MCPサーバーとして起動する

HTTP版はクライアントに子プロセスとして起動させず、先にGUIアプリを起動します。
待受先はlocalhost固定で、既定endpointは次のとおりです。
...
```powershell
.\Bin\SimpleMCPGraphicsSampleHttpD3D12\x64\Debug\SimpleMCPGraphicsSampleHttpD3D12.exe
.\Bin\SimpleMCPGraphicsSampleHttpVulkan\x64\Debug\SimpleMCPGraphicsSampleHttpVulkan.exe
```

portを変更する場合は`--port 18765`、ブラウザー系クライアントのOriginを追加する場合は
`--allow-origin https://example.test`を指定します。bind先を`0.0.0.0`へ変更する機能はありません。
HTTPクライアントが切断してもGUIは終了せず、ウィンドウを閉じるとHTTP listenerも終了します。
接続手順とwire仕様は[Streamable HTTP MCP](Docs/http-mcp.md)を参照してください。
...
- [stdio版とStreamable HTTP版の違い](Docs/mcp-transports.md)
- [Streamable HTTP MCP](Docs/http-mcp.md)
